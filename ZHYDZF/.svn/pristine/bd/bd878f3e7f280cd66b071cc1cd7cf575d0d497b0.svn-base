...
#import "BaseRecordLandscapeViewController.h"
...
@interface BaseRecordLandscapeViewController ()
...
@implementation BaseRecordLandscapeViewController
@synthesize currentdata,alertType,tableName,isHisRecord,dicWryInfo,isDTBD;
@synthesize recordStatus,popRecordController,dwbh,wrymc,xczfbh,menuTagID,basebh,kckssj,showSaveTip,displayFromLocal,unCommitedBilu;
...
- (void)viewDidLoad
{
    ...
    if(unCommitedBilu || [basebh length] == 0){ //做笔录
        ...
        if ([dwbh length] > 0) {//企业已经确定，从任务或污染源详情做笔录
            NSMutableDictionary *params = [NSMutableDictionary dictionaryWithCapacity:5];
